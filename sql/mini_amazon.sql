...
(
    ...
    seller_id         int                  null,
    quantity          int                  null,
    is_save_for_later tinyint(1) default 0 not null
)
...
(
    ...
    rating            int                                 null,
    comment           text                                null,
    review_time       timestamp default CURRENT_TIMESTAMP not null comment '评论时间',
    helpfulness_count int       default 0                 null,
...
(
    ...
    constraint `product_inventory_product_id,seller_id_uindex`
...
create table user_review_helpfulness
(
    id         int auto_increment
        primary key,
    user_id    bigint     not null,
    ...
    constraint user_review_helpfulness_ibfk_1
        foreign key (user_id) references users (id),
    constraint user_review_helpfulness_ibfk_2
        foreign key (review_id) references product_reviews (id)
)
    collate = utf8mb4_unicode_ci;

create index review_id
    on user_review_helpfulness (review_id);

create index user_id
    on user_review_helpfulness (user_id);